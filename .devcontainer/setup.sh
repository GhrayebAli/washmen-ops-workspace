...
ENVSUBST_VARS=""
for var in GOOGLE_MAPS_KEY ALGOLIA_APP_ID ALGOLIA_API_KEY SENTRY_DSN E2E_CLIENT_ID E2E_CLIENT_SECRET ANTHROPIC_API_KEY WASHMEN_GITHUB_TOKEN VPN_PRIVATE_KEY VIEWS_PGPASSWORD REDSHIFT_WAREHOUSE_DB_PASSWORD INTERNAL_USER_AUTH_SALT GOOGLE_PLACES_API_KEY; do
  if [ -n "${!var}" ]; then
